...
class Service:
    task = [
        {
            'name':"part4_postech",
            'description':'[part 4] image inpainting task'
        }
    ]

    def __init__(self):
        self.service_model = ImComplete()
...
class ImComplete(object):
    ...
    def run(self, content):
        # input data
        imagestring = content['image_contents']
        maskstring = content['mask_contents']
        img = self.stringToImage(imagestring)
        img_mask = self.stringToImage(maskstring)
        scenegraph_input = content['scene_graph']

        # convert to PIL image
        img, img_mask = utils.convert_image(img), utils.convert_image(img_mask)
        original_size = img.size # (width, height)
        utils.get_caption(scenegraph_input)
        caption = utils.refine_caption(scenegraph_input, img_mask)
        ...
        img = self.pipe(
            prompt=caption, image=img, mask_image=img_mask, num_inference_steps=50, strength=1.0, guidance_scale=7.5, height=img.height, width=img.width
        ).images[0]
        ...
        imgout = utils.convert_image(img)

        imgoutstring = {'image_contents':self.imageToString(imgout)}
        if imagestring is None or scenegraph_input is None:
            return {
                    'error':"invalid query"
                }
        else:            
    ...
    def generateDummySceneGraph(self):
        return {                
            "image_id":1,
            "relationships":[
