...
class MITCOCODataset(Dataset):
    def __init__(self, set_name='train', transform=None, config_file = None):
        ...
        # self.instance_base_dir = os.path.join(self.coco_path,'annotations', 'instance', set_name+'2017')
        ...
        json_full_file_name = os.path.join(self.ann_base_dir, 'modified_data.json')

        with open(json_full_file_name, "r") as file:
            self.file_list = json.load(file)
        self.file_list = self.file_list[:int(len(self.file_list)*0.25)]
    ...
    def __getitem__(self, index):
        ...
        if not os.path.isfile(image_path):
            image_path = os.path.join(self.img_base_dir.replace('val2017', 'train2017'), file_base_name + '.jpg') 
        ...
        # image_path = os.path.join(self.img_base_dir.replace('new_val2017', 'train2017'), file_base_name + '.jpg') 
        # if os.path.isfile(image_path):
        #     detr_detection_output_path = os.path.join(self.detr_detect_predict_dir.replace('new_val2017', 'train2017'), file_annotation['score_obj_quries_bboxes']+'.npz')
        #     score_obj_quries_bboxes = np.load(detr_detection_output_path.replace('new_val2017', 'train2017'), allow_pickle=True)

        # else:
        #     image_path = os.path.join(self.img_base_dir.replace('new_val2017', 'val2017'), file_base_name + '.jpg') 
        #     detr_detection_output_path = os.path.join(self.detr_detect_predict_dir.replace('new_val2017', 'val2017'), file_annotation['score_obj_quries_bboxes']+'.npz')
        #     score_obj_quries_bboxes = np.load(detr_detection_output_path.replace('new_val2017', 'val2017'), allow_pickle=True)
        ...
        x1, y1, x2, y2 = missing_box[0]*0.95, missing_box[1]*0.95, missing_box[2]*1.05, missing_box[3]*1.05
        if x2 >= 1:
            x2 = 1
        ...
        if self.transform is None:
            ...
            # mask = mask.unsqueeze(0).repeat(bboxes.shape[0], 1)
...
class MITCOCODataModule(pl.LightningDataModule):
    ...
    def setup(self, stage):
        self.COCO_train = MITCOCODataset(set_name='train', transform=None, config_file=self.config_file)
        self.COCO_val = MITCOCODataset(set_name='val', transform=None, config_file=self.config_file)
        self.COCO_test = MITCOCODataset(set_name='val', transform=None, config_file=self.config_file)
