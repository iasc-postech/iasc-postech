...
def resize_for_stable_diffusion(image, use_nearest=False):
    """
    Resize an image to meet Stable Diffusion requirements:
    both width and height should be a multiple of 8, and at least one dimension should be larger than 512.

    :param image_path: Path to the input image.
    :param save_path: Path to save the resized image. If None, the image won't be saved.
    :return: PIL Image object of the resized image.
    """
...
def refine_caption(scene_graph, mask):
    ...
    if len(contained_objects) > 0:
        for obj in contained_objects:
            obj_caption = ""            
            if obj["predicate"] != []:
                obj_caption = obj["class_name"] + " with following attributes " + stringify_list(obj["predicate"]) + ", "
            if obj["rel_predicate"] != []:
                obj_caption = obj["class_name"] + " also has following relationships: " + relative_stringify_list(obj["rel_predicate"], obj["class_name"]) + ", "
            default_caption = default_caption + obj_caption
